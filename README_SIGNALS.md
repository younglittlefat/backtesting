# 每日交易信号生成系统

## 功能概述

每日交易信号生成系统帮助您在每天收盘后，根据双均线策略（或其他策略）自动分析股票池中的所有标的，生成买入/卖出信号和资金分配建议。

## 核心文件

- `generate_signals.py` - Python信号生成脚本
- `generate_daily_signals.sh` - Shell启动脚本（推荐使用）

## 快速开始

### 1. 每天收盘后生成交易信号

```bash
# 基础用法
./generate_daily_signals.sh --stock-list results/trend_etf_pool_20251107.csv

# 指定可用资金和目标持仓数
./generate_daily_signals.sh \
  --stock-list results/trend_etf_pool_20251107.csv \
  --cash 200000 \
  --positions 10

# 保存报告到文件
./generate_daily_signals.sh \
  --stock-list results/trend_etf_pool_20251107.csv \
  --output signals/daily_$(date +%Y%m%d).txt \
  --csv signals/signals_$(date +%Y%m%d).csv
```

### 2. 使用自定义策略参数

```bash
# 使用短期5日均线和长期20日均线
./generate_daily_signals.sh \
  --stock-list results/trend_etf_pool_20251107.csv \
  --n1 5 \
  --n2 20

# 注意：策略参数必须与回测时使用的参数一致！
```

## 信号类型说明

- **BUY（买入）** - 金叉信号，短期均线上穿长期均线
- **SELL（卖出）** - 死叉信号，短期均线下穿长期均线
- **HOLD_LONG（持有多头）** - 短期均线在长期均线上方，继续持有
- **HOLD_SHORT（持有空头）** - 短期均线在长期均线下方，不建议买入

## 输出示例

```
================================================================================
交易信号报告 - 2025-11-07 15:35:20
================================================================================

📊 信号统计
--------------------------------------------------------------------------------
  BUY: 3 只
  HOLD_LONG: 15 只
  HOLD_SHORT: 82 只

🔔 买入信号（金叉）
--------------------------------------------------------------------------------
  588170.SH
    当前价格: ¥1.234
    短期均线: 1.235
    长期均线: 1.220
    信号强度: 1.23%
    说明: 金叉买入信号！短期均线(10日)上穿长期均线(20日)

  589010.SH
    当前价格: ¥2.567
    短期均线: 2.570
    长期均线: 2.550
    信号强度: 0.78%
    说明: 金叉买入信号！短期均线(10日)上穿长期均线(20日)

💰 资金分配建议
--------------------------------------------------------------------------------
  总资金: ¥100,000.00
  分配资金: ¥98,500.00
  剩余资金: ¥1,500.00
  建议持仓数: 3

  具体买入建议:

  [1] 588170.SH
      买入价格: ¥1.23
      买入数量: 26500 股
      预计成本: ¥32,595.00
      仓位占比: 32.60%
      信号强度: 1.23%

  [2] 589010.SH
      买入价格: ¥2.57
      买入数量: 12800 股
      预计成本: ¥32,896.00
      仓位占比: 32.90%
      信号强度: 0.78%

  [3] 159366.SZ
      买入价格: ¥3.45
      买入数量: 9500 股
      预计成本: ¥32,775.00
      仓位占比: 32.78%
      信号强度: 0.56%
```

## 参数说明

### 基本参数（无状态模式）

- `--stock-list` - 股票列表CSV文件 ⭐ 必需
- `--strategy` - 策略名称（默认: sma_cross）
- `--cash` - 可用资金（默认: 100000）
- `--positions` - 目标持仓数量（默认: 10）
- `--n1`, `--n2` - 短期和长期均线周期
- `--cost-model` - 费用模型（默认: cn_etf）
- `--data-dir` - 数据目录（默认: data/csv/daily）
- `--output` - 报告输出文件；`--csv` - CSV输出路径

### 费用模型（用于资金分配计算）

| 模型 | 佣金 | 滑点 | 用途 |
|------|------|------|------|
| `default` | 0% | 0% | 无费用 |
| `cn_etf` | 0.01% | 0.01% | 中国ETF ⭐ 推荐 |
| `cn_stock` | 0.03% | 0.1% | 中国A股（含印花税） |
| `us_stock` | 0.1% | 0.05% | 美股 |

## 资金分配与计算

### 分配策略

系统采用**等权重分配**：
1. 筛选所有"BUY"信号的标的
2. 按信号强度排序（均线乖离%）
3. 取前 N 个标的（N = `--positions`）
4. 平均分配资金给这 N 个标的
5. 计算买入股数（向下取整到100股倍数）

### 买入数量公式

```python
cash_per_position = total_cash / n_positions
effective_cash = cash_per_position * (1 - commission - spread)
shares = int(effective_cash / price / 100) * 100
```

### 重要注意

- **A股最小交易单位是100股**，系统自动向下取整
- **交易成本已计入**，实际买入金额略小于分配资金
- **信号强度**越大表示均线乖离越大（参考指标）

## 高级用法

### 定时任务

使用cron每天15:30自动运行：

```bash
# 编辑crontab
crontab -e

# 添加定时任务（每个交易日15:30）
30 15 * * 1-5 cd /path/to/backtesting && ./generate_daily_signals.sh \
  --stock-list results/trend_etf_pool_20251107.csv \
  --output signals/daily_$(date +\%Y\%m\%d).txt \
  --csv signals/signals_$(date +\%Y\%m\%d).csv
```

### Python脚本直接调用

如果需要更多自定义，可以直接使用Python脚本：

```bash
/home/zijunliu/miniforge3/condabin/conda run -n backtesting \
  python generate_signals.py \
  --stock-list results/trend_etf_pool_20251107.csv \
  --cash 100000 \
  --positions 10 \
  --n1 15 \
  --n2 50 \
  --data-dir data/csv/daily
```

### 结合其他工具

```bash
# 生成信号并发送邮件通知
./generate_daily_signals.sh \
  --stock-list results/trend_etf_pool_20251107.csv \
  --output /tmp/signals.txt && \
  mail -s "今日交易信号" your@email.com < /tmp/signals.txt

# 生成信号并保存到历史记录
./generate_daily_signals.sh \
  --stock-list results/trend_etf_pool_20251107.csv \
  --output signals/history/$(date +%Y%m%d).txt
```

## 常见问题

| 问题 | 答案 |
|------|------|
| **为什么没有买入信号？** | 所有标的处于下降趋势或无金叉发生。检查数据是否已更新到最新交易日。 |
| **信号强度是什么？** | (短期均线 - 长期均线) / 长期均线 × 100%。绝对值越大，乖离越大。 |
| **为什么买入数量不整数？** | 系统根据：资金 ÷ 持仓数 - 手续费 ÷ 价格，向下取整到100股倍数。 |
| **策略参数怎么选？** | 用 `./run_backtest.sh --optimize` 优化股票池，取收益最高的参数组合。 |
| **可以自动交易吗？** | 本系统仅生成建议，需人工审核后在券商软件中下单。 |
| **如何减少风险？** | 从小资金开始测试（1-2万），定期核对持仓与账户，监控实际表现。 |

## 技术细节

### 信号检测逻辑

```python
if sma_short_prev <= sma_long_prev and sma_short > sma_long:
    signal = 'BUY'      # 金叉
elif sma_short_prev >= sma_long_prev and sma_short < sma_long:
    signal = 'SELL'     # 死叉
elif sma_short > sma_long:
    signal = 'HOLD_LONG'    # 多头持有
else:
    signal = 'HOLD_SHORT'   # 空头
```

### 数据要求

- **最少数据量**：至少30天（用于计算均线）
- **默认回看期**：250个交易日（约1年）
- **数据格式**：Date, Open, High, Low, Close, Volume

## 🚀 持续交易系统（Portfolio Management）

### 问题分析

无状态系统每次运行都假设从零开始建仓。实际问题：
- 第1天：建议买A、B、C（各3.3万）
- 第2天：A出现卖出信号，但系统仍建议买D、E、F（假设还有10万）
- ❌ **错误**：系统不知道已持有A或剩余现金

**解决**：引入持仓状态文件记录当前真实状况，支持增量决策。

### 系统工作流

1. **读取持仓** → 获取当前持仓和现金
2. **生成信号** → 分析股票池（使用复权价格计算指标）
3. **增量决策** → 优先卖出（释放资金），再买入（使用释放资金）
4. **确认执行** → 用户确认后更新持仓
5. **记录交易** → 保存交易历史和更新后的持仓

#### 持仓状态文件格式

**`positions/portfolio.json`** - 核心状态文件

```json
{
  "cash": 35000.00,
  "positions": [
    {
      "ts_code": "159001.SZ",
      "shares": 10000,
      "entry_price": 3.25,
      "entry_date": "2025-11-06",
      "cost": 32512.50
    },
    {
      "ts_code": "510300.SH",
      "shares": 5000,
      "entry_price": 4.15,
      "entry_date": "2025-11-07",
      "cost": 20770.00
    }
  ],
  "last_update": "2025-11-07 15:35:20"
}
```

**字段说明：**
- `cash`: 当前可用现金
- `positions`: 持仓列表
  - `ts_code`: 股票代码
  - `shares`: 持有股数
  - `entry_price`: 买入均价
  - `entry_date`: 建仓日期
  - `cost`: 持仓成本（含手续费）
- `last_update`: 最后更新时间

#### 增量决策逻辑

系统按以下顺序执行：

1. **卖出处理**：遍历持仓，SELL信号的标的卖出，释放资金
2. **买入筛选**：从股票池中筛选BUY信号且未持有的标的，按信号强度排序
3. **可用资金计算**：当前现金 + 卖出收入 = 可用资金
4. **买入执行**：等权分配给筛选出的标的（最多不超过max_positions - 当前持仓数）
5. **持仓维护**：HOLD信号的标的继续持有，不操作

#### 使用指南

**1. 初始化（第一次使用）**

```bash
# 创建初始持仓状态文件
./generate_daily_signals.sh \
  --init 100000 \
  --portfolio-file positions/portfolio.json
```

输出：
```
✓ 持仓状态已初始化
  初始资金: ¥100,000.00
  持仓文件: positions/portfolio.json
```

**2. 每日分析（查看模式）**

```bash
# 分析当前信号，但不执行交易
./generate_daily_signals.sh \
  --stock-list results/trend_etf_pool_20251107.csv \
  --portfolio-file positions/portfolio.json \
  --analyze
```

输出示例：
```
================================================================================
当前持仓状态
================================================================================

💰 资金信息
--------------------------------------------------------------------------------
  可用现金: ¥35,000.00
  持仓市值: ¥65,000.00
  总资产:   ¥100,000.00
  持仓盈亏: +¥2,500.00 (+4.00%)

📊 持仓明细 (2/10)
--------------------------------------------------------------------------------
  159001.SZ
    持仓数量: 10000 股
    买入价格: ¥3.25 (2025-11-06)
    当前价格: ¥3.35
    持仓成本: ¥32,512.50
    当前市值: ¥33,500.00
    盈亏:     +¥987.50 (+3.04%)

  510300.SH
    持仓数量: 5000 股
    买入价格: ¥4.15 (2025-11-07)
    当前价格: ¥4.30
    持仓成本: ¥20,770.00
    当前市值: ¥21,500.00
    盈亏:     +¥730.00 (+3.51%)

================================================================================
交易建议
================================================================================

📉 卖出操作 (1)
--------------------------------------------------------------------------------
  [1] 159001.SZ
      操作: 卖出
      价格: ¥3.35
      数量: 10000 股
      预计收入: ¥33,466.50
      原因: 死叉卖出信号！短期均线(10日)下穿长期均线(20日)

📈 买入操作 (1)
--------------------------------------------------------------------------------
  [1] 588170.SH
      操作: 买入
      价格: ¥1.23
      数量: 55600 股
      预计成本: ¥68,388.00
      信号强度: 1.23%
      原因: 金叉买入信号！短期均线(10日)上穿长期均线(20日)

================================================================================
```

**3. 执行交易（执行模式）**

```bash
# 确认无误后，执行交易并更新持仓
./generate_daily_signals.sh \
  --stock-list results/trend_etf_pool_20251107.csv \
  --portfolio-file positions/portfolio.json \
  --execute
```

交互式确认：
```
⚠️  即将执行交易操作，请确认：
  - 卖出 1 只标的
  - 买入 1 只标的

是否确认执行？(yes/no): yes

开始执行交易...
✓ 卖出: 159001.SZ 10000股 @¥3.35 收入¥33,466.50
✓ 买入: 588170.SH 55600股 @¥1.23 成本¥68,388.00

✓ 持仓已更新: positions/portfolio.json
✓ 交易记录已保存: positions/history/trades_20251107.json
```

**4. 查看持仓（状态模式）**

```bash
# 随时查看当前持仓状态
./generate_daily_signals.sh \
  --portfolio-file positions/portfolio.json \
  --status
```

#### 完整命令参数

```bash
./generate_daily_signals.sh \
  --stock-list <股票池CSV> \              # 必需
  --portfolio-file <持仓JSON> \           # 持仓状态文件路径
  --strategy <策略名> \                    # 默认sma_cross
  --n1 <短期> --n2 <长期> \                # 策略参数
  --max-positions <数量> \                 # 最大持仓数（默认10）
  --cost-model <模型> \                    # 费用模型（默认cn_etf）
  [--init <初始资金>] \                    # 初始化模式
  [--analyze] \                           # 分析模式（不执行）
  [--execute] \                           # 执行模式（更新持仓）
  [--status]                              # 查看持仓状态
```

#### 典型使用场景

**场景1：完全新手第一次使用**

```bash
# Day 0: 初始化持仓，投入10万
./generate_daily_signals.sh --init 100000 --portfolio-file positions/portfolio.json

# Day 1: 第一天收盘后分析
./generate_daily_signals.sh \
  --stock-list results/trend_etf_pool.csv \
  --portfolio-file positions/portfolio.json \
  --analyze

# 确认后执行
./generate_daily_signals.sh \
  --stock-list results/trend_etf_pool.csv \
  --portfolio-file positions/portfolio.json \
  --execute
```

**场景2：已有持仓，导入现有仓位**

```bash
# 手动编辑 positions/portfolio.json，录入当前真实持仓
vim positions/portfolio.json

# 然后正常使用
./generate_daily_signals.sh \
  --stock-list results/trend_etf_pool.csv \
  --portfolio-file positions/portfolio.json \
  --analyze
```

**场景3：每日操作流程（最常用）**

```bash
# 每天15:30后，一键分析+执行
./generate_daily_signals.sh \
  --stock-list results/trend_etf_pool.csv \
  --portfolio-file positions/portfolio.json \
  --analyze && \
./generate_daily_signals.sh \
  --stock-list results/trend_etf_pool.csv \
  --portfolio-file positions/portfolio.json \
  --execute
```

**场景4：定时任务自动化**

```bash
# crontab定时任务（仅分析模式，发送通知）
30 15 * * 1-5 cd /path/to/backtesting && \
  ./generate_daily_signals.sh \
    --stock-list results/trend_etf_pool.csv \
    --portfolio-file positions/portfolio.json \
    --analyze \
    --output /tmp/signals.txt && \
  mail -s "今日交易信号" your@email.com < /tmp/signals.txt
```

#### 文件结构

```
backtesting/
├── generate_signals.py              # 信号生成脚本（增强版）
├── generate_daily_signals.sh        # 统一入口脚本（端到端）
├── README_SIGNALS.md               # 本文档
├── positions/                       # 持仓管理目录 ⭐ NEW
│   ├── portfolio.json              # 当前持仓状态文件
│   └── history/                    # 交易历史记录
│       ├── trades_20251106.json    # 每日交易记录
│       ├── trades_20251107.json
│       └── portfolio_backup/       # 持仓备份（可选）
├── signals/                        # 信号输出目录
│   ├── daily_20251107.txt
│   └── signals_20251107.csv
└── data/                           # 数据目录
    └── csv/daily/
```

#### 技术实现要点

| 功能 | 说明 |
|------|------|
| **持仓状态** | JSON格式存储，人工可读，每次交易后自动更新 |
| **增量决策** | 优先卖出→再买入→考虑持仓上限和资金约束 |
| **交易历史** | 每日记录到 `positions/history/trades_YYYYMMDD.json`，便于复盘 |
| **安全机制** | 执行模式需用户确认；分析模式只读不写；状态模式只查看 |
| **错误处理** | 持仓文件不存在→提示初始化；现金不足→拒绝买入；卖出不存在持仓→警告 |
| **向后兼容** | 不用 `--portfolio-file` 时行为与原系统一致；可渐进式升级 |

#### 双价格模式（自动处理）

- **信号计算**：使用复权价格（确保技术指标准确）
- **交易执行**：使用原始价格（符合实盘交易）
- **自动映射**：系统自动处理复权价格和原始价格的映射
- **价格差异示例**：588170.SH在2025-11-03 复权价格¥2.10，原始价格¥1.39（差异51%）

---

## 🎯 系统验收状态

**验收日期**: 2025-11-08

### 功能验证

| 功能 | 状态 | 说明 |
|------|------|------|
| 初始化模式 (`--init`) | ✅ | 创建初始持仓文件 |
| 状态查看 (`--status`) | ✅ | 实时查看持仓和盈亏 |
| 分析模式 (`--analyze`) | ✅ | 生成交易建议（推荐用于实盘） |
| 执行模式 (`--execute`) | ✅ | 执行交易并更新持仓（双价格模式已修复） |
| 盈亏计算 | ✅ | 自动计算持仓市值和盈亏百分比 |
| 增量决策 | ✅ | 基于当前持仓智能生成交易计划 |
| 向后兼容 | ✅ | 无状态模式完全保留 |
| 费用一致性 | ✅ | 与backtesting框架完全一致 |

### 推荐使用方式

**方式1: 分析模式（推荐）**
```bash
./generate_daily_signals.sh --analyze \
  --stock-list results/trend_etf_pool.csv \
  --portfolio-file positions/portfolio.json
```

**方式2: 分析+执行（自动化）**
```bash
./generate_daily_signals.sh --analyze ... && \
./generate_daily_signals.sh --execute ...
```

**方式3: 无状态模式（原有功能保留）**
```bash
./generate_daily_signals.sh --stock-list pool.csv --cash 100000
```

### 注意事项

⚠️ **重要提醒：**

1. **持仓文件与账户同步** - 每次真实交易后核对并更新，如果实际成交价格或数量不同
2. **100%成交假设** - 系统默认订单完全成交，实际需手动核对
3. **限制场景** - 不支持T+0日内交易、多账户、融资融券
4. **定期备份** - 建议用git管理 `positions/` 目录，每周备份一次
5. **从小资金开始** - 建议初始投入1-2万进行测试，监控实际表现

## 支持与反馈

如有问题或建议，请查看：
- 项目文档：`README_BACKTESTING.md`
- 回测文档：`README_BACKTESTING.md`
- ETF筛选文档：`etf_selector/README.md`
