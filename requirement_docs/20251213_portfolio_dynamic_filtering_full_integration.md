...
**验收进度更新（2026-02-02）**  
- 动态轮动已落地：新增 `rotation.enabled/schedule_path` 配置和 `DynamicPoolPortfolioRunner`，轮动日强制卖出非池内持仓并按新池重平衡。  
- 单元覆盖：`test_dynamic_pool_runner.py` 验证轮动表加载/池替换/卖出标签；`test_config_loader.py` 验证动态模式下的配置互斥与必填。  
- 性能/全量兼容性尚未实测；需后续跑全量回归与大样本回测确认。
